...
def strategy():
    ...
    print("資料更新完畢，最新的資料: ")
    print(real_time_data1.tail(1))
    print()
    print(real_time_data2.tail(1))
    print('-' * 50)
    ##################################################
    ...
    global open_long_ready, is_squeeze
    ...
    if current_position_type == 'None' and open_long_ready:
        if latest_j > 20 and latest_close > latest_open and latest_rsi > 20:
            trader.buy(size = 1)
            sl_price = min(real_time_data1.Low.iloc[-i] for i in range(1, 6))
            sell_ready = False
            print("開多單")
            return
    ################################################
    ...
    if current_position_type == 'long':
        if latest_j > 80 and latest_rsi > 80:
            sell_ready = True
            print("準備平倉")
            return
    ################################################
    ...
    if current_position_type == 'long':
        if sell_ready and (latest_j <= 80 or latest_rsi <= 80):
            trader.sell(size = 1)
            sell_ready = False
            print("平倉")
            return
